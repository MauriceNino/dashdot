...
You can also provide multiple interface names as a comma-separated list. When multiple interfaces are set, dash.
aggregates their live transfer rates in the network widget and shows each selected interface in the type label.
