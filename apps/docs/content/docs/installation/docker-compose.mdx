...
GPU support for Intel GPUs is available with another image tag. The `/dev/dri` device must be passed through to the container.
...
services:
  dash:
    image: mauricenino/dashdot:intel
    restart: unless-stopped
    privileged: true
    devices:
      - /dev/dri:/dev/dri
